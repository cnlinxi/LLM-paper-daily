...
def search_huggingface_model(query, max_results=2):
    ...
    Args:
        query (str): Keywords related to the paper.

    Returns:
        list: A list of dictionaries containing paper titles and URLs.
        [{"title": "google-bert/bert-base-uncased", "url": "https://huggingface.co/google-bert/bert-base-uncased"}]
    """
    params = {
        "search": query,
        "limit": max_results  # Number of results to retrieve
    }

    try:
        response = requests.get(hf_url, params=params)
...
def get_hf_model(paper_title):
    """Try my best to get HuggingFace models
    ...
    if not paper_title:
        return []
    # full text search
    hf_model_infos = search_huggingface_model(paper_title, max_results=1)
    if hf_model_infos:
        return hf_model_infos
    # possible model name
    if ":" in paper_title:
        possible_model_name = paper_title.split(":")[0]
...
def get_daily_papers(topic, query="slam", max_results=2):
    ...
    content = dict()
    content_to_web = dict()
    search_engine = arxiv.Search(query=query,
    ...
    for result in search_engine.results():
        ...
        try:
            ...
            hf_model_url = ""
            if hf_model_infos:
                hf_model_info = hf_model_infos[0]
                if "url" in hf_model_info:
                    hf_model_url = hf_model_info["url"]
            ...
            content[paper_key] = f"|**{update_time}**|**{paper_title}**|{paper_first_author} et.al.|[{paper_key}]({paper_url})|{content_repo_placeholder}|{content_hf_model_placeholder}|\n"
            content_to_web[paper_key] = f"- {update_time}, **{paper_title}**, {paper_first_author} et.al., Paper: [{paper_url}]({paper_url}){content_web_repo_placeholder}{content_web_hf_model_placeholder}"
...
def json_to_md(filename,
    ...
    with open(md_filename, "a+") as f:
        ...
        if show_badge == True:
            f.write(
                f"[![Contributors][contributors-shield]][contributors-url]\n")
        ...
        if show_badge == True:
            ...
            f.write((f"[contributors-url]: https://github.com/cnlinxi/"
                     f"llm-arxiv-daily/graphs/contributors\n"))
            f.write((
                f"[forks-shield]: https://img.shields.io/github/forks/cnlinxi/"
                f"llm-arxiv-daily.svg?style=for-the-badge\n"))
            f.write((f"[forks-url]: https://github.com/cnlinxi/"
                     f"llm-arxiv-daily/network/members\n"))
            f.write((
                f"[stars-shield]: https://img.shields.io/github/stars/cnlinxi/"
                f"llm-arxiv-daily.svg?style=for-the-badge\n"))
            f.write((f"[stars-url]: https://github.com/cnlinxi/"
                     f"llm-arxiv-daily/stargazers\n"))
            f.write((
                f"[issues-shield]: https://img.shields.io/github/issues/cnlinxi/"
                f"llm-arxiv-daily.svg?style=for-the-badge\n"))
            f.write((f"[issues-url]: https://github.com/cnlinxi/"
                     f"llm-arxiv-daily/issues\n\n"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--update_paper_links',
                        default=False,
                        action="store_true",
